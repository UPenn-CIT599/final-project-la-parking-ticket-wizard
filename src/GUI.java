...
//    HashMap<Integer, ParkingTickets> data= FH.getParkingTicketsRaw();
   Location userLocation=new Location(X,Y);
	...
	public void start(Stage primaryStage) throws Exception {
		window = primaryStage;
		window.setTitle("Parking Wizard For City of Los Angeles");
		...
		Scene bigDataScene=new Scene(BigDataPane,800,800);
		...
		predictButton.setOnAction(e ->{
	        ...
	        System.out.println("X Value: "+X+"   Y Value: "+Y); //TEST
				...
				GUIMessageBox.display("Wrong Data Entry","Please enter ONLY number for"
						+ " X,Y Coordinates, Day of Week and Hour of Day!");
			}
			if(DAY>=1&& DAY<=7&&HOUR>=0&&HOUR<=24) {
				inputValid=true;
			} else {GUIMessageBox.display("Wrong Data Entry","Hour Of Day should be between 0 "
					+ "and 24, Day of Week should be between 1 and 7!");}
...
//		File imageFile=new File ("PieChartForViolationByHour.png");
//        final Image image2 = new Image(imageFile);
//        imv.setImage(image2);
//
//        final HBox pictureRegion = new HBox();
//        
//        pictureRegion.getChildren().add(imv);
//        BigDataPane.add(pictureRegion, 1, 1);
		...
		GridPane.setConstraints(backToPrediction, 1, 0);
		backToPrediction.setOnAction(e -> window.setScene(scene));
		BigDataPane.getChildren().addAll(backToPrediction);
		
		grid.getChildren().addAll(xLabel, xInput, yLabel, yInput, dayLabel, day, 
				hourLabel, hour, predictButton, BigDataButton);
